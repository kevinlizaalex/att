...
?>  
              ...
              while ($row =mysqli_fetch_assoc($result))
              {
                echo "
                    <tr>
                        ...
                        {
                            echo "<td><form method=post>
                                <button value='".$row['id']."' name=ap disabled=true>Aprove</button> <button value='".$row['id']."' name=re>Reject</button>
                                </form></td>";
                        }
                        else if($row['status'] == 0)
                        {
                            echo "<td><form method=post>
                                <button value='".$row['id']."' name=ap>Aprove</button> <button value='".$row['id']."' name=re disabled=true>Reject</button>
                                </form></td>";
                        }
                        else
                        {
    ...
    if(isset($_POST['ap']))
    {
        include "include/dbconnect.php";
        $id = $_POST['ap'];
        $sql = "UPDATE `tbl_connection` SET `status`= 2 WHERE id = $id";
        $result=mysqli_query($conn,$sql);
    }
    if(isset($_POST['re']))
    {
        include "include/dbconnect.php";
        $rid = $_POST['re'];
        $rsql = "UPDATE `tbl_connection` SET `status`= 0 WHERE id = $rid";
